...
public class PredicateObjectMap extends R2RMLResource {
	...
	private static Logger logger = LogManager.getLogger(PredicateObjectMap.class);
	...
	public boolean preProcessAndValidate() {
		...
		/*
		 * PredicateObjectMaps must have one or more PredicateMaps and one or more
		 * ObjectMaps. Since we generate classes via the shorthands before processing,
		 * we just check rr:predicateMap and rr:objectMap.
		 */
		...
		for (Statement s : listo) {
			...
			/*
			 * Because of all the OWL axioms, it is difficult to infer whether resources are
			 * a ObjectMap or a RefObjectMap. This is because after reasoning, those
			 * instances are members of concepts that are OM or ROM concept union (!)
			 * something else. One should be able to deduce that from the roles they play
			 * however. TODO: investigate if we can't configure the reasoner and execute
			 * more elegantly.
			 */
			boolean isOM = r.hasProperty(R2RML.column) || r.hasProperty(R2RML.constant) || r.hasProperty(R2RML.template)
					|| r.hasProperty(RRF.functionCall) || r.hasProperty(RDF.type, R2RML.ObjectMap);
			boolean isROM = r.hasProperty(R2RML.joinCondition) || r.hasProperty(RDF.type, R2RML.RefObjectMap);
