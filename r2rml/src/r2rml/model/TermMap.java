...
public abstract class TermMap extends R2RMLResource {
	 ...
	 * mapping for each value and blank node. We will thus map the outcome of the
	 * constant, template, or column to the same blank node. In other words, if two
	 * TermMaps use "test"^^xsd:string, return same blank node. "1"^^xsd:string and
	 * "1"^^xsd:integer are different.
	 * 
	 */
	...
	/**
	 * Returns RDF term if the conditions for constant values for one of the
	 * TermMap's subclasses are met. Only to be called if a TermMap has a constant.
	 * 
	 * @return
	 */
	...
	public RDFNode generateRDFTerm(Row row) throws R2RMLException {
		...
		if (value == null) {
			...
			// iri.isAbsolute allows spaces, use Jena's IRIFactory to check whether the IRI
			// is valid
		...
		/*
		 * Otherwise, if the term type is rr:BlankNode: Return a blank node that is
		 * unique to the natural RDF lexical form corresponding to value.
		 */
		...
		/*
		 * Otherwise, if the term type is rr:Literal: 1. If the term map has a specified
		 * language tag, then return a plain literal with that language tag and with the
		 * natural RDF lexical form corresponding to value. 2. Otherwise, if the term
		 * map has a non-empty specified datatype that is different from the natural RDF
		 * datatype corresponding to the term map's implicit SQL datatype, then return
		 * the datatype- override RDF literal corresponding to value and the specified
		 * datatype. Otherwise, return the natural RDF literal corresponding to value.
		 */
			...
			if (datatype != null) {
				...
				// TODO: Ensure integers are mapped onto xsd:integer, but there must be a more
				// elegant way than via a string...
	...
	private Object getValueForRDFTerm(Row row) throws R2RMLException {
		if (isConstantValuedTermMap()) {
			...
			for (String reference : getReferencedColumns()) {
				...
				// If the term type is rr:IRI, then replace the pair of curly braces with an
				// IRI-safe
